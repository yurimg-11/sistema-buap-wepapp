...
export class AdministradoresService {
  ...
  public obtenerAdminPorID(idAdmin: number): Observable<any> {
    ...
    return this.http.get<any>(`${environment.url_api}/admins-edit/?id=${idAdmin}`, { headers });
  }
...
public eliminarAdmin(idAdmin: number): Observable<any> {
    const token = this.facadeService.getSessionToken();
    let headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token });
   return this.http.delete<any>(`${environment.url_api}/admins-edit/?id=${idAdmin}`, { headers });
  }
